...
class VideoQADataset(Dataset):
    ...
    def __init__(
        ...
        self.max_qa_length = (
            37  # 20 for MSRVTT, MSVD, TGIF-QA Trans & Action, 37 for nextqa
        )
        self.use_bbox = True
        ...
        if not self.multi_choice:
            ...
            self.ans_set = [
                "<unk>"
            ] + ans_set  # index 0 is reserved for out-of-vocab answer
        ...
        if self.use_bbox:
            bbox_feat_file = osp.join(
                self.video_feature_path,
                "region_feat_n/region_16c20b_{}.h5".format(mode),
            )
            ...
            with h5py.File(bbox_feat_file, "r") as fp:
                ...
                print(
                    feats.shape
                )  # v_num, clip_num, frame_per_clip, region_per_frame, feat_dim
                bboxes = fp["bbox"]
                for id, (vid, feat, bbox) in enumerate(zip(vids, feats, bboxes)):
                    self.bbox_feats[str(vid)] = (
                        feat[:, :, : self.bbox_num, :],
                        bbox[:, :, : self.bbox_num, :],
                    )  # (clip, frame, bbox, feat), (clip, frame, bbox, coord)

        if self.use_frame:
            app_feat_file = osp.join(
                video_feature_path, "frame_feat/app_feat_{}.h5".format(mode)
            )
        ...
        if self.use_mot:
            mot_feat_file = osp.join(
                video_feature_path, "mot_feat/mot_feat_{}.h5".format(mode)
            )
    ...
    def get_multi_choice_sample(self, idx):
        ...
        video_name, qns, ans, qid = (
            str(cur_sample[vid]),
            str(cur_sample["question"]),
            int(cur_sample["answer"]),
            str(cur_sample[qid]),
        )
    ...
    def __getitem__(self, idx):
        ...
        if self.multi_choice:
            (
                ...
                width,
                height,
            ) = self.get_multi_choice_sample(idx)
        else:
            cur_sample = self.sample_list.loc[idx]
            video_name, qns, ans, qid = (
                str(cur_sample["video"]),
                str(cur_sample["question"]),
                str(cur_sample["answer"]),
                str(cur_sample["qid"]),
            )
...
class QALoader:
    ...
    def train(self, mode):
        # print("Now in train")
        training_set = VideoQADataset(
            self.video_feature_path,
            self.video_feature_cache,
    ...
    def validate(self, mode):
        ...
        print(
            "Eligible video-qa pairs for validation/test : {}".format(
                len(validation_set)
            )
        )
        if not self.multi_choice and not self.use_bert:
