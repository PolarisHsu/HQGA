...
class EncoderVid(nn.Module):
    def __init__(
        ...
        if self.use_framePos:
            input_dim += pos_hidden
            self.framePos = positionalencoding1d(
                pos_hidden, self.num_frame
            ).cuda()  # (fnum, pos_dim)
            self.framePos = self.framePos.unsqueeze(1).expand(
                -1, self.num_bbox, -1
            )  # (fnum, num_bbox_perf, pos_dim)

        self.tohid = nn.Sequential(
            nn.Linear(input_dim, self.dim_hidden), nn.ELU(inplace=True)
        )
        ...
        self.app_conv = nn.Sequential(
            nn.Conv1d(feat_dim, self.dim_hidden, 3, padding=1), nn.ELU(inplace=True)
        )
    ...
    def forward(self, vid_feats):
        ...
        roi_bbox = vid_feats[0][:, :, :, :, dim_feat : (dim_feat + self.dim_bbox)].view(
            [batch_size, num_clip * frame_pclip, region_pframe, -1]
        )
        bbox_features = roi_feat

        if self.use_bboxPos:
            bbox_pos = self.bbox_conv(roi_bbox.permute(0, 3, 1, 2)).permute(0, 2, 3, 1)
            bbox_pos = bbox_pos.view(
                [batch_size, num_clip, frame_pclip, region_pframe, -1]
            )
